...
def main():
    ...
    training_data_dir = global_data_path+"word_level/"
    labelled_dir = training_data_dir+"/Labelled.pickle"
    unlabelled_dir = training_data_dir+"/Unlabelled.pickle"
...
#     Labelled_data = Labelled_data[0:1000]
    ...
    with open("./train_data.pickle", 'wb') as handle:
        # pickle.dump(Labelled_data+Unlabelled_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
        pickle.dump(Labelled_data, handle, protocol=pickle.HIGHEST_PROTOCOL)

    command = "python -u train.py full_initial "+str(A2L_Flag)+" "+str(MAX_LENGTH)+" "+str(VOCAB_SIZE)+" "+GPU_NUM
