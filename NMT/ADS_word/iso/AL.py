...
def initialise_GV():
    ...
    encoder = Encoder(vocab_inp_size, GLOVE_EMBEDDING_DIM, units, 100)
    decoder = Decoder(vocab_tar_size, GLOVE_EMBEDDING_DIM, units, 100)
...
def load_glove_embeddings(data_path):
    ...
    # with open("./glove_data.pickle", 'wb') as handle:
...
def get_glove_embeddings_for_sentences(line):
    ...
    return np.expand_dims(np.array(line_embeddings), axis=0)


def siamese_embeddings_util(AL_samples,eval_batch_size,mode="normal"):
    if mode == "init":
        print("Instantiating model")
        inp = tf.cast(np.zeros((1,2,GLOVE_EMBEDDING_DIM)), tf.float32)
        with tf.device('/device:GPU:0'):
            Siamese_Model.get_lstm_output(inp)
        print("Done")
        return
    ...
    siamese_embedlist = []	    
    for i, sample in enumerate(AL_samples):
        embedded_input_sample = get_glove_embeddings_for_sentences(sample[0])
        if not len(embedded_input_sample.shape) == 3:
        	embedded_input_sample = tf.cast(np.zeros((1,1,GLOVE_EMBEDDING_DIM)), tf.float32)
        with tf.device('/device:GPU:0'):
            siamese_embedded_sample = Siamese_Model.get_lstm_output(embedded_input_sample)
        siamese_embedlist.append(siamese_embedded_sample[0])
        if i % 1000 == 999:
        	print("Batch",(i//1000)+1,"done")
...
def main():
    ...
    # with open("./chosen_data2.pickle", 'rb') as handle:
    ...
    if A2L_Flag:
        ...
        siamese_embeddings_util([], 1, mode="init")
