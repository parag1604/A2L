...
def initialise_GV():
    ...
    tokenizer_base_dir = global_data_path+"word_level/"
    tokenizer_en_dir = tokenizer_base_dir+'tok_en.pickle'
    tokenizer_es_dir = tokenizer_base_dir+'tok_es.pickle'
    ...
    encoder = Encoder(vocab_inp_size, GLOVE_EMBEDDING_DIM, units, BATCH_PER_REPLICA)
    decoder = Decoder(vocab_tar_size, GLOVE_EMBEDDING_DIM, units, BATCH_PER_REPLICA)
...
def sick_filter_max_length(x, y, z, max_length=SIAMESE_MAX_LENGTH*GLOVE_EMBEDDING_DIM):
    return tf.logical_and(tf.size(x) <= max_length,
                          tf.size(y) <= max_length)

def sick_generate_dataset():

    global sick_glove_encoded

    tot_len = len(sick_glove_encoded)
    for i in range(tot_len):
        yield (sick_glove_encoded[i][0], sick_glove_encoded[i][1] ,  sick_glove_encoded[i][2]) 

def load_glove_embeddings(data_path):
    global glove_embeddings
    # with open(data_path, 'r', encoding='utf8') as glove_lines:
    #     for line in glove_lines:
    #         line = line.split()
    #         glove_embeddings[line[0]] = np.array(line[1:], dtype=np.float32)
    with open(data_path, 'rb') as handle:
        glove_embeddings = pickle.load(handle)
    print(len(glove_embeddings),"glove embeddings loaded")

def get_glove_embeddings_for_sentences(line):

    global glove_embeddings, tokenize

    line_embeddings = []

    for token in tokenize(re.sub(r"[' ]+[ ']", " ", line.lower())):
        try:
            vec = glove_embeddings[token]
            line_embeddings.append(vec)
        except KeyError:
            pass

    return np.array(line_embeddings)


def get_siamese_data():

    global SICK_DATA, sick_glove_encoded, GLOVE_CORPUS_SIZE, GLOVE_EMBEDDING_DIM

    # glove_path = global_data_path+"glove/glove."+str(GLOVE_CORPUS_SIZE)+"B."+str(GLOVE_EMBEDDING_DIM)+"d.txt"
    glove_path = global_data_path+"glove/glove_data.pickle"
    ...
    for data_tuple in SICK_DATA:
        src = get_glove_embeddings_for_sentences(data_tuple[0])
        tgt = get_glove_embeddings_for_sentences(data_tuple[1])
        scr = data_tuple[2]
        sick_glove_encoded.append((src, tgt, scr))

    sick_encoded_examples = tf.data.Dataset.from_generator(
                                sick_generate_dataset,
                                (tf.float32, tf.float32, tf.float32), 
                                ((None, GLOVE_EMBEDDING_DIM), (None, GLOVE_EMBEDDING_DIM),()))
    # sick_dataset = sick_examples.map(sick_tf_encode)
    sick_dataset = sick_encoded_examples.filter(sick_filter_max_length) # 9840 items

    # cache the dataset to memory to get a speedup while reading from it.
    sick_dataset = sick_dataset.cache()
    sick_dataset = sick_dataset.shuffle(SIAMESE_BUFFER_SIZE,seed=7).padded_batch(SIAMESE_BATCH_SIZE,
                        padded_shapes=((None, GLOVE_EMBEDDING_DIM), (None, GLOVE_EMBEDDING_DIM),()) )
    sick_dataset = sick_dataset.prefetch(tf.data.experimental.AUTOTUNE)
    # for data in sick_dataset:
    #     zeros = tf.constant(np.zeros(GLOVE_EMBEDDING_DIM), tf.float32)
    #     mask = tf.cast(tf.math.not_equal(data[0][0], zeros), tf.int32)
    #     mask = tf.cast(tf.math.not_equal(tf.math.reduce_sum(mask, axis=1), 0), tf.bool)
    #     print(mask)
    #     # print(data[0][0])
    #     break
    # sys.exit(35)
...
class Siamese(tf.keras.Model):
    ...
    def call(self, inp1, inp2):
        ...
        # maska = tf.cast(tf.math.not_equal(inp1, self.zeros), tf.int8)
        # mask1 = tf.cast(tf.math.not_equal(tf.math.reduce_sum(maska, axis=1), 0), tf.bool)
        # maskb = tf.cast(tf.math.not_equal(inp2, self.zeros), tf.int8)
        # mask2 = tf.cast(tf.math.not_equal(tf.math.reduce_sum(maskb, axis=1), 0), tf.bool)
        # mask1 = tf.cast(tf.math.not_equal(inp1, 0), tf.bool)
        # mask2 = tf.cast(tf.math.not_equal(inp2, 0), tf.bool)
        # bidir_output1 = self.bidir(inp1,training=True,mask=mask1)
        # bidir_output2 = self.bidir(inp2,training=True,mask=mask2)
        bidir_output1 = self.bidir(inp1,training=True)
        bidir_output2 = self.bidir(inp2,training=True)
    ...
    def get_lstm_output(self,inp1):
        ...
        # mask = tf.cast(np.zeros(SIAMESE_MAX_LENGTH), tf.bool)
        # mask[:inp1.shape[0]] = True
        # bidir_output1 = self.bidir(inp1,training=False,mask=mask)
        bidir_output1 = self.bidir(inp1,training=False)
...
def siamese_loss_function(real, pred):
    ...
    # return tf.reduce_mean(abs_loss)
...
siamese_train_step_signature = [
    tf.TensorSpec(shape=(None, None, GLOVE_EMBEDDING_DIM), dtype=tf.float32),
    tf.TensorSpec(shape=(None, None, GLOVE_EMBEDDING_DIM), dtype=tf.float32),
...
def siamese_train_step(inp1, inp2, lab):
    ...
    for i in range(len(gradients)):
        gradients[i] = tf.clip_by_value(gradients[i], -0.01, 0.01)
...
def load_siamese_optimizers():
    ...
    siamese_optimizer = tf.keras.optimizers.Adam(0.0003) #previously 10^-5
...
def create_siamese_model():
    ...
    Siamese_Model = Siamese(0,0,GLOVE_EMBEDDING_DIM,300)
...
def main():
    ...
    if train_type=="full_initial":
        ...
        print("Full training of Iso Siamese with glove embeddings")
        ...
        print("Full Training LSTM 20 Iterations")
        load_optimizers()
        train_data = []
        with open("./train_data.pickle", 'rb') as handle:
            train_data = pickle.load(handle)
        ...
        print("Incremental Training LSTM 3 Iterations")
        build_model()
        load_optimizers()
